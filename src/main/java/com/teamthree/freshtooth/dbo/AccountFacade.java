...
public class AccountFacade extends AbstractAccount<Account> {
    ...
    private static final String SQL_REGISTER_USER = "INSERT INTO Accounts(UserID, FullName, UserPassword, UserEmail, DefaultAvatar, ColorAvatar) VALUES(?, ?, ?, ?, ?, ?)";
    private static final String SQL_STATUS_ACCOUNT = "UPDATE Accounts SET UserStatus = ? WHERE UserEmail = ?";
    ...
    private static final String SQL_GET_STATISTIC_ACCOUNT = "SELECT COUNT(*) FROM Accounts WHERE UserRole = 0 AND YEAR(AccountCreated) = ? GROUP BY MONTH(AccountCreated)";
    ...
    @Override
    protected boolean registerAccount(Connection connection, Account account) throws SQLException {
        try {
            if (connection != null) {
        ...
        try {
            if (connection != null) {
                switch (object.toString()) {
                    case "Disable":
                        preparedStatement = connection.prepareStatement(SQL_STATUS_ACCOUNT);
    ...
    @Override
    protected List<Integer> statisticAccount(Connection connection, Object object) throws SQLException {
        ArrayList<Integer> statisticAccountList = new ArrayList<>();
        ...
        try {
            if (connection != null) {
                ...
                resultSet = preparedStatement.executeQuery();
                while (resultSet.next()) {
                    statisticAccountList.add(resultSet.getInt(1));
                }
            }
        } finally {
            if (resultSet != null) {
